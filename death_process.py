""" Deep Adaptive Design for Death Process """
import torch
...
def train(cfg, logger, model, experiment, batch_size, L, M, T, max_epoch, verbose=10):
    ...
    """
    optimizer = torch.optim.Adam(model.parameters(), lr=cfg.lr, betas=(0.9, 0.999), weight_decay=0)
    scheduler = torch.optim.lr_scheduler.ExponentialLR(optimizer, gamma=0.96)

    criterion = PCELossScoreGradient(L, T, experiment.log_likelihood)
    ...
    for epoch in range(max_epoch):
        ...
        for step in range(max_step):
            # sample true theta
            theta_0 = experiment.sample_theta((batch_size,))                        # [B, 1]
...
@hydra.main(version_base=None, config_path="./config", config_name="dp")
def main(cfg):
    logger = create_logger(os.path.join(cfg.output_dir, 'logs'), name='death_process')
    ...
    if cfg.model.name == 'MLP':
        encoder = EncoderNetwork(cfg.data.dim_design, cfg.data.dim_outcome, hidden_dim=cfg.model.hidden_dim, encoding_dim=cfg.model.encoding_dim, hidden_depth=cfg.model.encoder_hidden_depth, activation=nn.Softplus())
        emitter = EmitterNetwork(cfg.model.encoding_dim, cfg.data.dim_design, hidden_dim=cfg.model.hidden_dim, hidden_depth=cfg.model.emitter_hidden_depth, activation=nn.Softplus(), output_nonlinearity=nn.Softplus())
    ...
    # Data
    experiment = DeathProcess(theta_dist=cfg.data.theta_dist, N=cfg.data.N)
    ...
    # Evaluation
    # NOTE: here we simply use the sPCE bound, however, it is possible to compute the sEIG to high accuracy using numerical integration
