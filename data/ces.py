...
from .extra_dist import CensoredSigmoidNormal
...
class CES(BED):
    ...
    def __init__(
            self,
            dim_design: int = 6,  # dimension of input (2 baskets x 3 commodities)
            dim_outcome: int = 1,  # dimension of outcome (preference rating)
            design_scale: int = 100,  # scale of the design space (baskets in [0,100])
            outcome_scale: float = 1,  # scale of the experiment outcomes
            noise_scale: float = 0.005,  # sigma_eta in the model
            epsilon: float = 2 ** (-22),  # epsilon for response clipping
            **kwargs
    ) -> None:
        super(CES, self).__init__(design_scale, outcome_scale)

        self.dim_design = dim_design
    ...
    def sample_theta(self, batch_size):
        ...
        # if reshape:
        #     theta = theta.reshape(*shape, self.n_theta, 1)  # [(L), B, 5, 1]
    ...
    def utility(self, x, rho, alpha):
        ...
        """
        # Calculate CES utility: U(x) = (sum_i alpha_i * x_i^rho)^(1/rho)
        
        x = torch.clamp(x, 0.01, 100.0)
    ...
    def __str__(self) -> str:
        ...
        for key in del_keys:
            del info[key]
        return f"CES({', '.join('{}={}'.format(key, val) for key, val in info.items())})"